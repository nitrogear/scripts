...
aem_aas_install 'aem-install' do
  ...
  java_opts node['aem']['author']['java_opts'] unless node['aem']['author']['java_opts'].nil?
  action :install
end

node['aem']['author']['validation_urls'].each do |url, word|
...
aem_aas_packages 'aem-install-packages' do
  port node['aem']['author']['port']
  base_packages node['aem']['author']['additional']['install_packages']
  current_version_packages node['aem']['author']['6.1.0']['install_packages']
  action :install
end
